...
def fetch_gitignore_template(template: str) -> str:
    ...
    response = set(item.strip().lower() for line in response for item in line.split(","))
    available_templates = response

    template = template.lower()

    if template not in available_templates:
        raise ValueError(f"Template '{template}' not found in available templates.")
...
if __name__ == "__main__":
    mcp.run(transport="streamable-http", host="localhost", port=8000, path="/mcp/")